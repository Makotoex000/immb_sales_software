...
class VendaServiceClass {
  async criarVenda(dto: CreateVendaDTO): Promise<Venda> {
    ...
    await pool
      ...
      .query(`
        INSERT INTO Vendas (id, total, formaPagamento, dataVenda, status)
        VALUES (@id, @total, @formaPagamento, @dataVenda, 'concluida')
      `);
    ...
    for (const item of dto.itens) {
      ...
      await pool
        ...
        .query(`
          INSERT INTO ItensVenda (id, vendaId, produtoId, nomeProduto, quantidade, valorUnitario, subtotal)
          VALUES (@id, @vendaId, @produtoId, @nomeProduto, @quantidade, @valorUnitario, @subtotal)
        `);
  ...
  async obterVendasPorData(data: Date): Promise<Venda[]> {
    ...
    const result = await pool
      ...
      .query(`
        SELECT * FROM Vendas WHERE dataVenda >= @dataInicio AND dataVenda <= @dataFim ORDER BY dataVenda DESC
      `);
  ...
  async desfazerVenda(id: string): Promise<boolean> {
    ...
    // Obter venda para recuperar os itens
    ...
    for (const item of venda.itens) {
      await pool
        .request()
        .input('id', sql.NVarChar(50), item.produtoId)
        .input('quantidade', sql.Int, item.quantidade)
        .query(`
  ...
  async deletarVenda(id: string): Promise<boolean> {
    ...
    await pool
      ...
      .input('vendaId', sql.NVarChar(50), id)
      .query(`
        DELETE FROM ItensVenda WHERE vendaId = @vendaId
      `);
  ...
  private async obterItensPorVendaId(vendaId: string): Promise<ItemVenda[]> {
    ...
    const result = await pool
      .request()
      .input('vendaId', sql.NVarChar(50), vendaId)
      .query(`
        SELECT * FROM ItensVenda WHERE vendaId = @vendaId
      `);
